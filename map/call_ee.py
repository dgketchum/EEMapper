...
RF_TRAINING_POINTS = 'projects/ee-dgketchum/assets/points/IrrMap_RF_points_mgrs'
...
MISSING = {'CA': [2000, 2007, 2014], 'CO': [1998, 2003, 2006, 2013, 2016], 'MT': [2010, 2013]}
...
def get_sr_series(tables, out_name):
    ...
    pt_ct = 0
    for state, years in MISSING.items():
        for year in years:
            for mgrs in WESTERN_11_MGRS:
                name_prefix = '{}_{}_{}_{}'.format(out_name, state, mgrs, year)
                ...
                roi = ee.FeatureCollection(MGRS)
                roi = roi.filter(ee.Filter.eq('MGRS_TILE', mgrs))

                roi_state = ee.FeatureCollection(os.path.join(BOUNDARIES, state))
                ...
                table = table.filterBounds(roi_state)
                table = table.filter(ee.Filter.eq('MGRS_TILE', mgrs))
                points = table.size().getInfo()
                if points == 0:
                    print(state, mgrs, points)
                    continue

                print('{} points: {}'.format(points, name_prefix))
