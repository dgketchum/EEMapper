...
class MLPWithEmbeddings(nn.Module):
    def __init__(self, input_dim_continuous, cat_dims, output_dim, emb_dropout=0.1):
        ...
        self.embedding_layers = nn.ModuleList([nn.Embedding(num_embeddings, min(50, (num_embeddings + 1) // 2)) for num_embeddings in cat_dims])
...
def train_model(past_processed_dir, modern_processed_dir, categorical_json_path, batch_size=64, epochs=20):
    ...
    predict_modern_data(model, modern_processed_dir, batch_size, all_feature_names, categorical_mappings)

def predict_modern_data(model, modern_processed_dir, batch_size, all_features, categorical_mappings):
    modern_dataset = BandsDataset([os.path.join(modern_processed_dir, f) for f in
                                  os.listdir(modern_processed_dir) if f.endswith('.parquet')], all_features, categorical_mappings)
...
if __name__ == '__main__':
    ...
    train_model(past_dir, modern_dir, categorical_map)
